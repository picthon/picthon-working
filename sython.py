...
async def autoname_loop():
    ...
    while AUTONAMESTART:
        ...
        try:
            ...
            await sython(functions.account.UpdateProfileRequest(last_name=name))
...
async def autobio_loop():
    ...
    while AUTOBIOSTART:
        ...
        try:
            ...
            await sython(functions.account.UpdateProfileRequest(about=bio))
...
@sython.on(admin_cmd(pattern=f"{phow8t}(?:\s|$)([\s\S]*)"))
...
@sython.on(admin_cmd(pattern=f"{namew8t}(?:\s|$)([\s\S]*)"))
...
@sython.on(admin_cmd(pattern=f"{biow8t}(?:\s|$)([\s\S]*)"))
...
sython.loop.create_task(digitalpicloop())

sython.loop.create_task(autoname_loop())

sython.loop.create_task(autobio_loop())
...
async def _(event):
    ...
    await sython.send_file(
        "me", pic, caption=f"owner  -  @a_t_9 تـم حفظ الصورة او الفيديو الذاتي هنا : ذمه و أمانه ليوم الدين الي يستخدم الأمر للابتزاز"
...
![https://telegra.ph/file/b550f8bdd9c73adf26649.jpg](PICTHON)
 𝘱𝘪𝘤𝘵𝘩𝘰𝘯 - 𝘣𝘭𝘢𝘤𝘬 ま
- البنك : `{res}`
**"""
                     )
...
async def _(event):
        ...
        await sython.send_message('@zmmbot', 'جاري التجميع بواسطة | PICTHON')
        channel_entity = await sython.get_entity(bot_username)
        ...
        for i in range(100):
            ...
            if msgs.message.find('لا يوجد قنوات في الوقت الحالي , قم يتجميع النقاط بطريقه مختلفه') != -1:
                await sython.send_message(event.chat_id, f"لايوجد قنوات  في البوت | PIC")
                break
